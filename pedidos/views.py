from django.shortcuts import render, redirect, get_object_or_404, RequestContext # solo RequestContext de m
from jsonview.decorators import json_view #linea completa de m
from pedidos import forms, models, utils #solo utils de m
from organizaciones.models import Farmacia #linea completa de m
from django.contrib.auth.decorators import login_required

from django.db.models import Q # Herramienta que nos permite "darle de comer" expresiones con OR a los filter

from crispy_forms.utils import render_crispy_form #linea completa de m
from django.contrib.auth import decorators as authd #linea completa de m
from medicamentos.models import Medicamento #linea completa de m
import datetime #linea completa de m
import re #linea completa de m
...
def verificarPedidosDeFarmacia(pkLaboratorio): #***********************************************************************
    pkLab = pkLaboratorio
    ...
    for pendientes in pendientesDeFarmacias:
        renglonesPedidoFarmacia = models.DetallePedidoDeFarmacia.objects.filter( Q(pedidoFarmacia=pendientes.pk) & Q( estaPedido=False ) & Q( medicamento = pkLab ))
    ...
    if request.method == "POST":
        ...
        if '_agregar' in request.POST :
            if detallePedidoLab_form.is_valid():
                ...
                renglones.append({ "medicamento": detpedido.medicamento.pk, "nombre": detpedido.medicamento.nombreFantasia.nombreF,  "cantidad": detpedido.cantidad, "cantidadPendiente": detpedido.cantidad })
        ...
        verificarPedidosDeFarmacia(unLaboratorio.pk)#para procesar los pedidos de farmacia
